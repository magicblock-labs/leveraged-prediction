...
describe("chart geometry", () => {
  it("maps the forty-second window exactly onto the bottom time axis", () => {
    const now = 1_000_000;
    ...
    expect(geometry.x(now - 20_000)).toBeCloseTo(geometry.plotLeft);
    expect(geometry.x(now)).toBeCloseTo(geometry.plotLeft + geometry.plotWidth / 2);
    expect(geometry.x(now + 20_000)).toBeCloseTo(geometry.plotRight);
    expect(geometry.timeOffsetAt(geometry.plotLeft)).toBeCloseTo(-20_000);
    expect(geometry.timeOffsetAt(geometry.plotRight)).toBeCloseTo(20_000);
  });
