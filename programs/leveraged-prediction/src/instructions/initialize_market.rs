...
pub fn handler(ctx: Context<InitializeMarket>, sponsor_lamports: u64) -> Result<()> {
    if ctx.accounts.market.collateral_mint != Pubkey::default() {
...
pub struct InitializeMarket<'info> {
    ...
    #[account(init_if_needed, payer = admin, space = 8 + Market::INIT_SPACE, seeds = [MARKET_SEED, collateral_mint.key().as_ref()], bump)]
